# وحدة محلل الجمهور المستهدف

> تربط الوحدة توقعات الجمهور بسير الحبكة وتقترح تعديلات لتعزيز الجاذبية السوقية.

## 🎯 نطاق التحليل
- **مجال التحليل الرئيس:** `audience`
- **الفئات المتأثرة:** `market_positioning`
...
1. **details[].insights[]**: تحليل توقعات الجمهور وتوزيع الاهتمام عبر المشاهد.
1. **details[].insights[].recommendedActions**: خطوات تسويقية أو سردية لضمان الارتباط العاطفي.

## 🧾 مثال JSON معتمد
```json
{
  "title": "تقرير محلل الجمهور المستهدف",
  "summary": "يقارن توقعات الجمهور مع بنية الحبكة الحالية، ويقترح إيقاعاً أسرع للفصل الأول للمحافظة على الاهتمام.",
  "metadata": {
  ...
  "recommendations": [
    {
      "id": "target_audience_analyzer_rec_1",
    ...
    {
      "id": "target_audience_analyzer_rec_2",
  ...
  "next_steps": {
    "immediate": [
      {
        "id": "target_audience_analyzer_action_1",
    ...
    "upcoming": [
      {
        "id": "target_audience_analyzer_action_2",
